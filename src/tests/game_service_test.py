...
class FakeUserRepository:
    ...
    def find_by_username(self, username):
        user_list = list(filter(lambda user: user.username == username, self.users))
...
class TestGameService(unittest.TestCase):
    ...
    def test_create_user(self):
        ...
        self.assertRaises(UsernameExistsError, lambda: self.game_service.create_user(self.user_u1.username, 'password'))
    ...
    def test_login(self):
        ...
        self.assertRaises(InvalidCredentialsError, lambda: self.game_service.login(user.username, 'wrong'))
